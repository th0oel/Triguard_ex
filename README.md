...
TriGuard AI는 병무청·질병관리청·방위사업청의 공공데이터를 결합하여 14개 지방병무청 권역별 병력운용 위험도를 실시간으로 산출하는 Streamlit 대시보드입니다. 인력 가용성·감염병 확산·군수물자 조달이라는 세 축을 하나의 통합 Risk Score로 정량화하며, 위험 등급별 rule-based 대응 가이드 및 RandomForest 기반 AI 예측 분석을 함께 제공합니다. 전 지표는 0–100 범위로 정규화되고, 0 나누기 방지 및 지역명 fuzzy matching이 자동 처리됩니다.
...
```
triguard-ai/
├── app.py               # Streamlit main app — 자동 로드·파싱·계산·렌더링 오케스트레이션
├── requirements.txt
├── data/                # CSV 데이터 폴더 (자동 감지)
└── modules/
...
**데이터 자동 로드**: `data/` 폴더에 CSV 파일을 넣으면 앱 시작 시 자동 감지합니다.  
**수동 업로드**: 사이드바에서 파일을 직접 업로드하거나 시뮬레이션 모드를 선택할 수 있습니다.
...
```
인력Risk = 0.9 × (0.30 × 입영감소율 + 0.25 × 검사감소율 + 0.20 × 면제율
                 + 0.10 × 재검율 + 0.15 × 차질률)
         + 0.10 × 인구보정점수
```
...
| 지표 | 산출식 |
|------|--------|
| 입영인원 감소율 | `(기준연도 현역 − 최신 현역) / 기준연도 현역 × 100 × (6 / 연수차)` |
| 병역판정검사 감소율 | `(기준연도 처분 − 최신 처분) / 기준연도 처분 × 100 × (6 / 연수차)` |
| 병역면제율 | `병역면제자 / 처분인원 × 100` |
| 재신체검사율 | `재신체검사 / 처분인원 × 100` |
| 입영 차질률 | `(행방불명 + 기피) / 입영실통지 × 100` |
| 인구 보정점수 | `(7.0 − 병역자원비율) / 7.0 × 40` — 20대 남성 비율 낮을수록 위험 |
...
| 구성요소 | 산출식 | 범위 |
|----------|--------|------|
| 발생률지수 (지방청별) | `(총발생률 − 800) / (1600 − 800) × 100` | 지방청별 상이 |
| 질병등급 가중합지수 | `ln(1 + 가중발생합계) / ln(10001) × 100` (1급×3, 2급×2, 3급×1) | 전국 단일 |
| 인플루엔자 강도지수 | `최신절기 최대분율 / 역사적최대 × 60` (미제공 시 30.0) | 전국 단일 |
| ARI 트렌드지수 | `현재연도 평균 / 역사적최대 × 60` (미제공 시 30.0) | 전국 단일 |
...
| 공급업체 집중도 | 입찰업체정보 있으면 `100 − 다양성지수`, 없으면 상위5 점유율 |
...
- `calc_manpower_risk(..., population_df)` — 지방청별 인력 Risk 산출. 6년 중장기 감소율 + 행정안전부 인구 보정(10%) 반영.
- `calc_disease_dc(..., jibang_disease_df)` — 지방청별 발생률지수를 계산 후 DC를 산출, `jibang_dc_df[지방청, 발생률지수, 감염병DC]`를 반환.
- `calc_material_risk(..., bidders)` — 물자 Risk 산출. 입찰업체 데이터 제공 시 HHI 기반 집중도, 없으면 상위5 점유율 사용.
